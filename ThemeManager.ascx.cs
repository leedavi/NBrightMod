        ...
        protected override void OnLoad(EventArgs e)
        {
            try
            {
                ...
                if (Page.IsPostBack == false)
                {
                    ...
                    var strOut = LocalUtils.RazorTemplRender("config.thememanager.cshtml", ModuleId.ToString(""), "configthememanager" + Utils.GetCurrentCulture(), settings, Utils.GetCurrentCulture(),true); // force debug mode, we don't want to cache.
                    var lit = new Literal();
                    lit.Text = strOut;
